...
def make_box(world,width,depth,height,wall_thickness=0.005,mass=float('inf')):
	...
	middle.loadFile("data/objects/cube.tri")
	...
	middle.transform([0.05,0,0,0,depth,0,0,0,0.5*height],[-0.05,-depth*0.5,0])
	...
	boxgeom = Geometry3D()
	boxgeom.setGroup()
	for i,elem in enumerate([left,right,front,back,bottom,middle]):
		g = Geometry3D(elem)
...
def launch_competition_balls(robotname,num_balls=20):
	...
	for layer in xrange(numlayers):
		...
		for i in xrange(ballsperlayer):
			...
			mass = ball.getMass()
			mass.setMass(mass.getMass()*pow(scale,3))
			ball.setMass(mass)
			R = so3.identity()
			x = i % w
			y = i / w
			x = (x - (w-1)*0.5)*box_dims[0]*0.9/(w)
			y = (y - (h-1)*0.5)*box_dims[1]*0.9/(h)
			t = [x,y,0.13 + layer*0.13]
			t[0] += random.uniform(-0.01,0.01)
			t[1] += random.uniform(-0.01,0.01)
